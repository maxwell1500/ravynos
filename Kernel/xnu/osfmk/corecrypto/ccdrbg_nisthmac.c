...
update(struct ccdrbg_state *ctx, unsigned ndata, ...)
{
	...
	uint8_t hmac_buf[512] CC_ALIGNED(8) = {0};
	cchmac_ctx_t hmac_ctx = (cchmac_ctx_t)hmac_buf;

	// NIST 800-90A §10.1.2.2: always run b=0 and b=1 passes when provided_data present;
	// skip the b=1 pass only when there is no provided_data (ndata==0).
	for (uint8_t b = 0; b < 2; b += 1) {
		// K = HMAC(K, V || b || provided_data)
		...
		// When no provided_data, b=0 pass is sufficient per spec step 3.
		if (b == 0 && ndata == 0) {
			break;
		}
	}

	cc_clear(sizeof(hmac_buf), hmac_buf);
}
...
generate(struct ccdrbg_state *ctx, size_t out_nbytes, void *out, size_t add_nbytes, const void *add)
{
	...
	while (out_nbytes > 0) {
		// Save current V, then advance: V = HMAC(K, V)
		cc_memcpy(Vprev, drbg_ctx->V, outlen);
		cchmac(info, outlen, drbg_ctx->key, outlen, drbg_ctx->V, drbg_ctx->V);

		// FIPS 140-2, 4.9.2: consecutive V values must differ
