...
The host toolchain (Default.xctoolchain) includes clang, LLVM, Apple cctools, xcbuild, and other utilities needed to build the OS. Building LLVM initially takes a __long__ time, so be patient. It lives in `<build_dir>/Developer/Default.xctoolchain` once built.
...
* Run: `SDKROOT=$(xcrun --sdk macosx --show-sdk-path) cmake -S /path/to/ravynos -B /path/to/build -GNinja`
* Run: `SDKROOT=$(xcrun --sdk macosx --show-sdk-path) cmake --build /path/to/build`
