...
.if defined(SDKROOT)
__toolpath	=	${SDKROOT}/../../Toolchains/Default.xctoolchain
TOOLCHAIN	?=	${__toolpath:tA}
.else
TOOLCHAIN	?=	/Library/Developer/Toolchains/Default.xctoolchain
.endif
...
CODESIGN_ALLOCATE	?=	${TOOLS}/codesign_allocate

# C Type Format data is required for DTrace
CTFFLAGS	?=	-L VERSION

CTFCONVERT	?=	${TOOLS}/ctfconvert
CTFMERGE	?=	${TOOLS}/ctfmerge
CTFINSERT	?=	${TOOLS}/ctfinsert
CTFDUMP		?=	${TOOLS}/ctfdump
...
MIG		?=	${TOOLS}/mig
MIGCC		?=	${CC}
MIGCOM 		?=	${TOOLCHAIN}/usr/libexec/migcom
...
OBJCXXFLAGS	?=	${OBJCINCLUDES} ${CXXFLAGS} -Wno-import
...
.if !defined(%POSIX)
SIZE		?=	size
STRIPBIN	?=	${TOOLS}/strip
.endif

UNIFDEF		?=	${TOOLS}/unifdef

XCRUN		?=	${TOOLS}/xcrun
XCBUILD		?=	${TOOLS}/xcbuild
