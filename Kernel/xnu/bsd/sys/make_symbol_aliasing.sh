...
if [ -z "${AVAILABILITY_PL}" ] || [ ! -x "${AVAILABILITY_PL}" ]; then
    if [ -x "${SDKROOT}/${DRIVERKITROOT}/usr/local/libexec/availability.pl" ]; then
        AVAILABILITY_PL="${SDKROOT}/${DRIVERKITROOT}/usr/local/libexec/availability.pl"
    elif [ -x "${SDKROOT}/usr/local/libexec/availability.pl" ]; then
        AVAILABILITY_PL="${SDKROOT}/usr/local/libexec/availability.pl"
    elif [ -x "$(dirname "$0")/../../../../Developer/ravynOS.sdk/usr/local/libexec/availability.pl" ]; then
        AVAILABILITY_PL="$(dirname "$0")/../../../../Developer/ravynOS.sdk/usr/local/libexec/availability.pl"
    fi
fi
