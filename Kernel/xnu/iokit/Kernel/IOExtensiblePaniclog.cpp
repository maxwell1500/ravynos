...
// Legacy IOKit API - matches IOKit/IOExtensiblePaniclog.h virtuals
kern_return_t
...
kern_return_t
IOExtensiblePaniclog::InsertData(OSData *data)
{
	if (data == nullptr) {
		return kIOReturnBadArgument;
	}
...
kern_return_t
IOExtensiblePaniclog::AppendData(OSData *data)
{
	if (data == nullptr) {
		return kIOReturnBadArgument;
	}
...
kern_return_t
IOExtensiblePaniclog::CopyMemoryDescriptor(IOBufferMemoryDescriptor **mem)
{
	(void) ext_paniclog_claim_buffer(extPaniclogHandle);
	if (mem == nullptr || iomd == nullptr) {
		return kIOReturnBadArgument;
	}
	iomd->retain();
	*mem = iomd;
