...
zone_element_size(void *elem, zone_t *z, bool clear_oob, vm_offset_t *oob_offs)
{
	...
	if (from_zone_map(addr, sizeof(void *))) {
		...
		end   = vm_memtag_canonicalize_address(addr + esize);
		offs  = 0;
...
zstack_tbi_fill(void *addr)
{
	...
	return vm_memtag_canonicalize_address(elem);
}
...
zone_tag_element(zone_t zone, vm_offset_t addr, vm_size_t elem_size)
{
	...
	if (zone->z_percpu) {
		zpercpu_foreach_cpu(index) {
			vm_memtag_set_tag(tagged_address + ptoa(index), elem_size);
		}
	}
...
zcram_memtag_init(zone_t zone, vm_offset_t base, uint32_t start, uint32_t end)
{
	...
	for (uint32_t i = start; i < end; i++) {
		vm_offset_t elem_addr = base + oob_offs + i * elem_size;
...
zone_cram_early(zone_t zone, vm_offset_t newmem, vm_size_t size)
{
	 ...
	 */
	vm_memtag_bzero((void *)newmem, size);
...
{
	return mach_vm_range_contains(&zone_info.zi_pgz_range, vm_memtag_canonicalize_address(addr));
}
...
zfree_percpu(union zone_or_view zov, void *addr)
{
	...
	zpercpu_foreach_cpu(i) {
		vm_memtag_bzero((char *)addr + ptoa(i), esize);
	}
	zfree_ext(zone, zstats, addr, ZFREE_PACK_SIZE(esize, esize));
}
...
#if KASAN_CLASSIC || CONFIG_PROB_GZALLOC || VM_TAG_SIZECLASSES
	vm_offset_t esize = zone_elem_inner_size(zone);
#endif
...
found:
	...
	if (zone->z_tbi_tag) {
		addr = vm_memtag_fixup_ptr(offs + zone_pva_to_addr(pva));
	} else {
...
zone_reclaim_recirc_trim(zone_t z, struct zone_depot *zd)
{
	for (;;) {
		...
		/*
		 * If the number of magazines to reclaim is too large,
		 * we might be keeping preemption disabled for too long.
		 *
		 * Drop and retake the lock to allow for preemption to occur.
		 */
		zone_unlock(z);
		zone_lock(z);
	}
...
zone_bootstrap(void)
{
	...
	const char *zb1 = "    zone_bootstrap: entered!\r\n";
	while (*zb1) { pal_serial_putc(*zb1++); }
	...
	const char *zb2 = "    zone_bootstrap: ml_early_cpu_max_number...\r\n";
	while (*zb2) { pal_serial_putc(*zb2++); }
	...
	const char *zb3 = "    zone_bootstrap: random_bool_init...\r\n";
	while (*zb3) { pal_serial_putc(*zb3++); }
	const char *zbe1 = "    zone_bootstrap: entering zpercpu_foreach_cpu...\r\n";
	while (*zbe1) { pal_serial_putc(*zbe1++); }

	zpercpu_foreach_cpu(cpu) {
		const char *zbc = "      zone_bootstrap: cpu loop...\r\n";
		while (*zbc) { pal_serial_putc(*zbc++); }
		random_bool_init(&zone_bool_gen[cpu].zbg_bg);
	}

	const char *zbe2 = "    zone_bootstrap: zpercpu_foreach_cpu done!\r\n";
	while (*zbe2) { pal_serial_putc(*zbe2++); }
...
#if ZSECURITY_CONFIG(SAD_FENG_SHUI)
	const char *zb4 = "    zone_bootstrap: SAD_FENG_SHUI...\r\n";
	while (*zb4) { pal_serial_putc(*zb4++); }
...
#endif /* ZSECURITY_CONFIG(SAD_FENG_SHUI) */
	const char *zb5 = "    zone_bootstrap: thread_call_setup_with_options...\r\n";
	while (*zb5) { pal_serial_putc(*zb5++); }

	const char *zb5a = "    zone_bootstrap: setting up zone_expand_callout...\r\n";
	while (*zb5a) { pal_serial_putc(*zb5a++); }
	thread_call_setup_with_options(&zone_expand_callout,
	    zone_expand_async, NULL, THREAD_CALL_PRIORITY_HIGH,
	    THREAD_CALL_OPTIONS_ONCE);

	const char *zb5b = "    zone_bootstrap: zone_expand_callout done! setting up zone_trim_callout...\r\n";
	while (*zb5b) { pal_serial_putc(*zb5b++); }
	thread_call_setup_with_options(&zone_trim_callout,
	    zone_trim_async, NULL, THREAD_CALL_PRIORITY_USER,
	    THREAD_CALL_OPTIONS_ONCE);

	const char *zb6 = "    zone_bootstrap: finished!\r\n";
	while (*zb6) { pal_serial_putc(*zb6++); }
}
...
zone_metadata_init(void)
{
	...
	for (uint32_t i = 0; i < atop(early_sz); i++) {
		...
		for (uint32_t eidx = 0; eidx < z->z_chunk_elems; eidx++) {
			...
			addr = vm_memtag_fixup_ptr(base + eidx * esize);
#if KASAN_CLASSIC
			kasan_alloc(addr,
...
zone_gc_stress_test_run(__unused int64_t in, int64_t *out)
{
	...
	for (uint64_t i = 0; i < in; i++) {
		size_t count = zc_mag_size() * zc_free_batch_size() * 10;
