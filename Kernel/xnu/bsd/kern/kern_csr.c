...
csr_bootstrap(void)
{
	...
	const char *c1 = "    csr_bootstrap: entered...\r\n";
	while (*c1) { pal_serial_putc(*c1++); }

	boot_args *args = (boot_args *)PE_state.bootArgs;
	if (args && (args->flags & kBootArgsFlagCSRBoot)) {
		/* special booter; allow everything */
		csr_allow_all = 1;
	}

	const char *c2 = "    csr_bootstrap: done!\r\n";
	while (*c2) { pal_serial_putc(*c2++); }
}
STARTUP(TUNABLES, STARTUP_RANK_FIRST, csr_bootstrap);
