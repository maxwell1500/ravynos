...
/*
 * ravynOS local addition: definition of the per-type trust cache
 * configuration table. In Apple's build this table lives in the closed
 * trust cache runtime; the ravynOS kernel provides it here. Types loaded
 * at boot carry a NULL entitlement value and cannot be loaded by userland
 * callers (the with-type loading interface rejects them outright).
 */
const TCTypeConfigEntry TCTypeConfig[kTCTypeTotal] = {
	[kTCTypeInvalid]        = { NULL },
	[kTCTypeStatic]         = { NULL },
	[kTCTypeEngineering]    = { NULL },
	[kTCTypeLegacy]         = { NULL },
	[kTCTypeCryptex1BootOS] = { NULL },
	[kTCTypeCryptex1BootApp] = { NULL },
	[kTCTypePersonalized]   = { "pmap" },
	[kTCTypeBaseSystem]     = { "pmap" },
};

/*
 * ravynOS local addition: trust cache runtime initialization. Apple's
 * closed trust cache runtime performs full runtime setup inside the trust
 * cache library; the ravynOS kernel records the boot-time policy flags so
 * the AMFI interface implementation (which owns the runtime structures)
 * can honor them.
 */
void
	...
	const img4_runtime_t *img4_runtime)
{
	...
	rt->allowSecondStaticTC = allow_second_static_cache;
	rt->allowEngineeringTC = allow_engineering_caches;
}
